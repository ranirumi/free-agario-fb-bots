...
// @author       Nel, xN3BULA, Jimboy3100, test114514
// @grant        none
...
function drawMinimap() {
    ...
    let xScaler = canvas.width / game.bx;
    let yScaler = canvas.height / game.bx;
    ctx.save();
    ...
    for (var i = 0; i < window.game.minimap.length; i++) {
        ...
        x = (game.bx / 2 + window.game.minimap[i].x - game.offsetX) / game.bx * canvas.width;
        y = (game.by / 2 + window.game.minimap[i].y - game.offsetY) / game.by * canvas.height;
        r = Math.max(window.game.minimap[i].size, 180) * (xScaler + yScaler) / 2;
        ctx.moveTo(x, y);
    ...
    ctx.beginPath();
    x = (game.bx / 2 + user.cellX - game.offsetX) / game.bx * canvas.width;
    y = (game.by / 2 + user.cellY - game.offsetY) / game.by * canvas.height;
    ctx.moveTo(x, y);
